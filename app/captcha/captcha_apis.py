...
def get_captcha(captcha_id: UUID, jwt_token=Security(APIKeyHeader(name="X-API-Key"))):
    ...
    return StreamingResponse(
        BytesIO(captcha_data["image"]), headers=headers, media_type="image/png"
    )


@router.get(
    "/get-answer/{captcha_id}/{answer}/",
    response_model=CaptchaCheckAnswer,
    tags=["captcha"],
)
def get_captcha_answer(
    captcha_id: UUID,
    answer: int,
    jwt_token=Security(APIKeyHeader(name="X-API-Key")),
):
    check_user_auth(jwt_token)
